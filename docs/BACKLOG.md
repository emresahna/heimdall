...
Canonical engineering roadmap. Generated 2026-08-03 per `.ai/prompts/02-backlog-generation.md`,
following `.ai/templates/backlog-template.md`.
...
Related history (read these first):

- `docs/REPORT.md` — full project review; source of most findings below.
- `docs/tracker-c-development.md` — kernel-side tracer assessment.

---

## Repository Snapshot (2026-08-03)

- **Stack:** Go 1.25.3 (`go.mod`), cilium/ebpf v0.20.0, ClickHouse client v2.42.0, gRPC, k8s client-go
  informers, embedded dependency-free web UI. `Dockerfile.agent` (debian-slim) + `Dockerfile.server` (scratch).
- **Agent:** 7 syscall tracepoints (`internal/bpf/tracker.c`): `write`/`sendto`/`writev` → requests,
  `read`/`recvfrom` (enter+exit) → responses. 128-byte sample into ringbuf; userspace correlates
  `(pid, fd)`, enriches from Kubernetes, batches over gRPC.
- **Server:** gRPC ingest → ClickHouse `MergeTree` (day partitions, 7-day fixed TTL), HTTP API + UI, `/metrics`.
- **Ops:** Docker Compose, raw k8s manifests, Helm chart, `local_test.sh` kind flow. 22 commits, no tags, no CHANGELOG.
- **CI:** `.github/workflows/ci.yml` — lint (golangci-lint v2), vet, `go test -race`, mod tidy check, build.
- **Tests:** 423 lines across parser/collector, config, correlator, pipeline parser, http server, circuit
  breaker. **No benchmarks. No e2e.**
- **Already resolved since `docs/REPORT.md`:** CI exists, MIT LICENSE added, Makefile CGO inconsistency
  fixed (`Makefile:24-29` — both `CGO_ENABLED=0`), `vmlinux.h` gitignored (commit `6b972f4`), BPF map
  pressure metrics + ClickHouse retry/backoff landed. Do not re-plan these.

---

## Baseline SLOs (to be measured, not yet defined)

No numeric targets exist anywhere in the repo. Targets gate every performance decision.

- Capture throughput target (events/s/node) — to be measured with EPIC-008 benchmarks on the target
  kernel; proposed starting point: sustain 10k events/s with <1% ringbuf drops.
- Ringbuf drops = 0 at reference rate; agent overhead <5% CPU at reference rate.
- End-to-end ingest p95 latency (agent sample → ClickHouse row) — proposed: <2s.
- Correlation match rate ≥ 95% of parsed responses at reference rate (measured via diagnostics counters).

---

## Roadmap Phases

| Phase | Theme | Epics | Gate |
|---|---|---|---|
| 0 | Hygiene | EPIC-001, EPIC-002 | — |
| 1 | Trustworthy data | EPIC-003 … EPIC-009 | — |
| 2 | Data usefulness | EPIC-010 … EPIC-013 | — |
| 3 | Product expansion | EPIC-014, EPIC-015 | ADR-001 (product direction) |
| 4 | Hardening & scale | EPIC-016 … EPIC-018 | — |

---

## ADRs

ADRs follow `.ai/templates/adr-template.md` (Context / Decision / Consequences). Recorded here as
proposed decisions; finalize as `docs/adr/NNN-*.md` during implementation.

### ADR-001 (blocking, decision required) — Product direction

**Context:** the capture model (syscall tracepoints, plaintext HTTP/1.x only) misses the majority of
real-world traffic. Three defensible directions exist (`docs/REPORT.md` §6.1): (a) lightweight
zero-instrumentation HTTP tracer for dev/kind clusters, (b) full k8s network observability
(TLS, HTTP/2, endpoints, scoping), (c) OTLP-compatible companion to existing stacks.

**Recommendation:** (b-lite) — stay on syscalls, ship endpoint metadata (EPIC-010) and cgroup scoping
(EPIC-005) now, then begin TLS visibility via uprobes (EPIC-014) incrementally. This preserves the
existing moat (fail-cheap in-kernel classification) while closing the two most embarrassing gaps.
**Consequences:** Phase 3 work is gated on this. Revisit when (b-lite) requires TLS; (c) is the
fallback if distribution becomes the priority.

### ADR-002 — Correlation key becomes `(pid, fd, seqno)`

**Context:** `(pid, fd)` mispairs under fd reuse (`internal/correlation/correlator.go:15`).
**Proposal:** per-`(pid, fd)` monotonically increasing `seqno` incremented in-kernel on each request
`emit_event`; userspace keys on `(pid, fd, seqno)`; responses match the head of the per-key queue.
**Consequences:** breaking proto/schema change; requires map-based per-key counter in `tracker.c`.

### ADR-003 — `payload` column

**Context:** column exists, never populated (`internal/storage/clickhouse.go:86`; dropped at
`internal/pipeline/processor.go:43-67`).
**Proposal:** implement opt-in body sampling (default off, `HTTP_SAMPLE_BYTES=0` = off) with
redaction hooks (EPIC-016); keep column. Reject "drop column" unless product says never body data.
**Consequences:** schema stays; adds optional cost only when enabled; redaction is a hard requirement
before any body data is stored.

### ADR-004 — Storage schema evolution

**Context:** `method/status/type` plain `String`, `ORDER BY (timestamp, pid, fd)`, fixed 7-day TTL,
`path LIKE '%x%'` unindexable (`internal/storage/clickhouse.go:76-97,215-217`).
**Proposal (EPIC-018):** `LowCardinality(String)` for `method/type/namespace/pod/node`, ORDER BY
`(namespace, pod, toStartOfHour(timestamp), pid, fd)` once cardinality data exists, configurable TTL,
data-skipping indexes on `path`. Migration via `ALTER TABLE` (pattern already exists at
`clickhouse.go:102-117`).
**Consequences:** requires re-partition/table replacement for ORDER BY change; TTL becomes env-configurable.
...
### EPIC-004 — Resolve the `payload` lie (ADR-003)
...
### 2026-08-03 — Initial generation (this file)

- Created per `.ai/prompts/02-backlog-generation.md` after repository audit (`.ai/prompts/01-repository-audit.md`).
- Findings sourced from `docs/REPORT.md`, `docs/tracker-c-development.md`, and direct verification of the
  current tree (evidence cited per epic).
- Deliberately excluded already-resolved REPORT items (CI, LICENSE, CGO, vmlinux.h gitignore) — see
  Repository Snapshot.
- Decisions deferred to ADR-001 (product direction) gate Phases 3-4 epics, per REPORT.md §6.1/§7.
- Epics ordered P0 → P2 by impact/ROI: correctness and hot-path health first (Phase 1), then data
  usefulness (Phase 2), expansion/hardening last.
